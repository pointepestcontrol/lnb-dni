...
class LnbDni {
    ...
    public function register_backend_scripts() {
        if (current_user_can('administrator')) {
            wp_register_style('dni-settings-styles', plugin_dir_url(__FILE__) . 'assets/css/settings-style.css');
    ...
    public function menu_page() {
        ...
        wp_enqueue_style('dni-settings-styles');
        wp_enqueue_script('dni-settings-js');

        ?>

<div class="lnbSettingsPage">
    <br />
    <h1 class="lnbSettingsPage__title">LeadsNearby - DNI Settings</h1>
    <p>Enter the tracking number and select the visitor source that should cause it to be displayed. The phone number
        will be displayed on the site the way it is formatted here</p>
    <i><b></b></i><br />
        ...
        <div class="rowLnbSettingContainer">
            ...
            <div class="rowLnbSetting sr<?php echo $i ?>" data-index="<?php echo $i ?>">
                ...
                <div class="lnbSetting setting-left">
                    <input required="" class="lnbSetting__field lnbSetting__field--text" name="option[<?php echo $i ?>][phone-number]"
                        type="tel" maxlength="14" value="<?php echo $setting['phone-number'] ?>">
